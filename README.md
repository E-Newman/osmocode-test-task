...
[CRouter.check_route_rules(\$rules)](./CRouter.php) - проверяет маршрут на соответствие правилам, указанным в массиве `$rules`.
...
1. Если включена отладка (`CDebug::isEnabled()`), то в стандартный вывод печатается сообщение `check_route_rules`, свойство `type` текущего правила и массив `$rules`;
...
6. если определено свойство правила `regexp`:
  - если значение проверяемого параметра соответствует регулярному выражению из свойства `regexp`, то метод возвращает `1`;
...
7. если свойство `type` определено, но не из списка `or, and. request_var, request_uri` и включена отладка, то в стандартный вывод печатается сообщение `unknown rule` и массив `$rules`;
...
Пусть задан следующий массив правил:
...
Пусть массив `$_REQUEST` поступившего запроса выглядит так:
